...
namespace MaxSumTriplet
{
    ...
    class Program
    {
        ...
        public int solve(List<int> A)
        {
            ...
            int[] leftMaxvals = new int[A.Count];
            int[] rightMaxvals = new int[A.Count];

            int i = A.Count - 2;
            int max = A[A.Count - 1];
            rightMaxvals[A.Count - 1] = max;

            while (i >= 0)
            {
                rightMaxvals[i] = Math.Max(max, A[i + 1]);
                max = rightMaxvals[i];
                i--;
            }
            ...
            while (i < A.Count)
            {
                A.Sort(0, i,  new Comparaer());
                leftMaxvals[i] = binarySearchJustSmallerNumber(A, 0, i - 1, A[i]);
                i++;
            }
            ...
            // Arrays.Print<int>(original);
            // Arrays.Print<int>(rightMaxvals);
            // Arrays.Print<int>(leftMaxvals);
            ...
            for (i = 1; i < A.Count - 1; i++)
            {
                if (leftMaxvals[i] != -1)
                {
                    if(leftMaxvals[i]!=-1 && rightMaxvals[i]!=-1 &&  original[i] + leftMaxvals[i] + rightMaxvals[i]> max && leftMaxvals[i]<original[i] && original[i]<rightMaxvals[i])
                    {
                        max = Math.Max(original[i] + leftMaxvals[i] + rightMaxvals[i], max);
                        Console.WriteLine(leftMaxvals[i]+" "+original[i] + " " + rightMaxvals[i]+" "+max);
                    }
        ...
        private int binarySearchJustSmallerNumber(List<int> a, int left, int right, int number)
        {
            int originalRight = right;
            while (left < right && left>=0 && right<a.Count)
            {
                
                int mid = (left + right) / 2;
                
                if (mid == originalRight)
                {
                    if(a[mid]< number)
                    {
                        return a[mid];
